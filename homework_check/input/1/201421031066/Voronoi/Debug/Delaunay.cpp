...
} VERTEX2D, *VERTEX2D_PTR;//表示一个点，x、y分别是点的坐标
...
} EDGE, *EDGE_PTR;//存储维诺边的数据结构，v1和v2分别是两个三角形外接圆圆心的坐标

typedef struct TRIANGLE_TYP
{
	int i1; // vertex index//点索引  
	int i2; 
	int i3; 
...
} TRIANGLE, *TRIANGLE_PTR;//Delaunay三角网的三角链表，i1、i2、i3分别是三角网每个三角形的三个端点索引号（vertex index）

typedef struct MESH_TYP
{
	int vertex_num;		//顶点数
	int triangle_num;	//三角数

	VERTEX2D_PTR pVerArr; // point to outer vertices arrary	// 指向每个顶点
	TRIANGLE_PTR pTriArr; // point to outer triangles arrary// 指向三角链表

} MESH, *MESH_PTR;//存储Delaunay三角网的数据结构
