...
int TiltCorrection(Mat src,Mat &dst,Rect &anchorRect,Point &originalPoint, double &angle)
{
	...
	Size seSize;
	Mat se;
	//Mat se = getStructuringElement(MORPH_CROSS,Size(2,2));
	//morphologyEx(bw,bw,cv::MORPH_OPEN,se);
	...
	if (contours.size()==1)
	{
		...
		seSize.height =MIN(anchorRect.height >>2,MAX(15,anchorRect.height >>3));
		...
		if (min(abs(minx.x-miny.x),abs(minx.y-miny.y))<3||dist<dist_thresh/*||(abs(rect.width-anchorRect.width)<4&&abs(rect.height-anchorRect.height)<4)*/)
		{
			angle=0;
			Rect rect=boundingRect(boundary);
