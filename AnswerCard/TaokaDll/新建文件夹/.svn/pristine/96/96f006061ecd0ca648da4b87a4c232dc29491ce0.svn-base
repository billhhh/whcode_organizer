	...
	if (acinfo.isAnchorPointExist)
	{
		...
		if(acinfo.markerInfo.xNum)
		{
			...
			if (returnValue)
			{
				...
				ATLTRACE("水平定位点检测异常，异常代码：%d\n",returnValue);
				return XMARKERS_ERR_BASE;
			}
		}
		//垂直标记点检测
		vector<PerMarker_info>::iterator iter = acinfo.yMarker.begin();
...
//		mbt.InitMarkersBoundaryTable(xMarkers,yMarkers,0.3,false);
	...
	if (acinfo.questioninfo.size() > 0 )
	{
		...
		if (acinfo.refPoint.size() > 0 )
		{
			...
			returnValue=DetectRefPoint(quesImg,srcYMarker,yMarkers,acinfo.refPoint,refMarker,rPRevise,refAngle);
			if(returnValue!=0)
			{
