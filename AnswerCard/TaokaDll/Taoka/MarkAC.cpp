	...
	*pResult=new AC_RST;
	if (actype==3||actype==4)
	{
		...
		for (int i=tkinfo3->tkinfo.size()-1;i>=0;i--)
		{
			...
			if(returnValue!=0&&returnValue<ANCHOR_POINT_ERR_BASE+9)
				continue;
			else
			{
	...
	else
	{
		...
		for (int i=tkinfo->tkinfo.size()-1;i>=0;i--)
		{
			...
			if(returnValue!=0&&returnValue<YMARKERS_ERR_BASE+4)
				continue;
			else
			{
	...
	*pRearResult=new AC_RST;
	...
	else
	{
		...
		for (int i=tkinfo->tkinfo.size()-1;i>=0;i--)
		{
			...
			if(returnValue!=0&&returnValue<YMARKERS_ERR_BASE+4&&returnValue!=REFPOINT_ERROR)
				continue;
			else
			{
