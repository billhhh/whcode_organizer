...
// 	threshold(dstImg,RoiImage,160,255,THRESH_BINARY_INV);	//转换为二值图像	
	...
	threshold(RoiImage,tmp,t*0.9,255,THRESH_BINARY_INV);
// 	imshow("t",tmp);
// 	waitKey(0);
	//闭操作
	Mat se = getStructuringElement(MORPH_ELLIPSE,Size(3,4));
	 morphologyEx(tmp,tmp,cv::MORPH_CLOSE,se);
	//开操作
	se = getStructuringElement(MORPH_RECT,Size(Achorsize.width*0.7,Achorsize.height*0.7));
...
// 	threshold(dstImg,RoiImage,160,255,THRESH_BINARY_INV);	//转换为二值图像	
	...
	morphologyEx(tmp,tmp,cv::MORPH_CLOSE,se);
	//开操作
	se = getStructuringElement(MORPH_RECT,Size(Achorsize.width*0.8,Achorsize.height*0.8));
	morphologyEx(tmp,tmp,cv::MORPH_OPEN,se);
// 	vector<vector<Point> > contours;
// 	findContours(bw.clone(),contours,CV_RETR_EXTERNAL,CV_CHAIN_APPROX_SIMPLE );
// 	for (int i=0;i<contours.size();i++)
...
// 		waitKey();
		...
		Mat roiImage2=srcimg(roiRect).clone();
		int err_ID = DetectAncorERR(roiImage2,DetectedRect,AnchorPos[i]);
		if (!err_ID)
		{
...
// 		imwrite("I:\\roi.jpg",roi);
// 		imwrite("I:\\bw.jpg",tmp);
...
// 	cvWaitKey();
	...
	int downHThreashold=MAX(5,abs(rectSize.height-markHight))*diff_threashold;
...
int DetectAnchor(vector<cv::Rect> &DetectedRect,int i,short anchorType,Size xMarkSize,Size yMarkSize)
{
	if(i==0)
	{
		if (abs(DetectedRect[1].x-DetectedRect[0].x) > abs(DetectedRect[1].y-DetectedRect[0].y))
		{
			SortRects(DetectedRect,true);
		}else
		{
	...
	else if ((i==1&&anchorType==13)||(i==2&&anchorType==7))
	{
		if (abs(DetectedRect[1].x-DetectedRect[0].x) > abs(DetectedRect[1].y-DetectedRect[0].y))
		{
			SortRects(DetectedRect,true);
