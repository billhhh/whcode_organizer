...
//检测答案  类型二、三
int DetectAnswer(Mat Img,vector<ItemPos> questionInfo,MarkersBoundaryTable &mbt,AnswerResult &ar,bool questionType,double sensibilityThresh,double areaThreshold,double seSize)
{
	//题目数量
	int questionNum=questionInfo.size();
	for(int i=questionNum-1;i>=0;i--)
	...
	/*	cout<<"questionId:"<<perQuestionInfo.id<<endl;*/
		...
		//检测结果
		//检测结果【对应的参数：1、小题矩形区域 2、是否横向 3、是否单选 4、选项个数 5、为0表示选择题中间没有ABCD，为1表示中间有ABCD 6、填涂一致性灵敏度 7、最小填涂阈值
		//8、是否调整形态学模板大小 为0不做调整  大于0则调大，小于0则调小 9、问题答案】
...
//检测答案 类型三多组
int DetectAnswer(Mat Img,vector<ItemPos> questionInfo,map<ushort,MarkersBoundaryTable*> &mbtMap,AnswerResult &ar,bool questionType,double sensibilityThresh,double areaThreshold,double seSize)
{
...
//检测结果【对应的参数：1、小题矩形区域 2、是否横向 3、是否单选 4、选项个数 5、为0表示选择题中间没有ABCD，为1表示中间有ABCD 6、填涂一致性灵敏度 7、最小填涂阈值 
//8、是否调整形态学模板大小 为0不做调整  大于0则调大，小于0则调小 9、问题答案】
int DetectPreQuestionAnswer( Mat roi,bool isCrosswise,bool isSingleChoice,int optionNum,bool quetionType,double sensibilityThresh,double areaThreshold,double seSizeTh,PerquestionResult &result)  //0.正常 1.没有填涂
{
	result.answer = 0;
	...
	//选项矩形
	Rect optionRect;
	double step;  //每个选项的宽度或高度，步长
	if(isCrosswise)
	{
		//如果选项是横向的
		step = (double)roi.cols/optionNum; //宽
		optionRect = Rect(0,0,(int)step,roi.rows);
	}else
	{
		//如果选项是竖向的
		step = (double)roi.rows/optionNum; //高
	...
	cv::Size seSize(3,3);
	if(quetionType)         //如果选择题中有字母  quetionType==1
	{
		seSize.width = MAX(6,(optionRect.width>>1)-(optionRect.width>>3));  //  (optionRect.width/2)-(optionRect.width/8)
	...
	/*	seSize.width = optionRect.width>>2;
		...
		{
			//调整形态学模板大小，>0则调大
			seSize.width = ((optionRect.width>>1)-(optionRect.width>>3)) *(1+seSizeTh);
			seSize.height= (optionRect.height>>2) *(1+seSizeTh/2);
		}
	}
	else  //如果选择题中没字母  quetionType==0
	{
		seSize.width = optionRect.width>>2;
		seSize.height= optionRect.height>>2;
		if (seSizeTh < 0)
...
// 		}
	...
	Mat se = getStructuringElement(MORPH_RECT,seSize); //! returns structuring element of the specified shape and size 返回特定大小的结构化的形状和大小
