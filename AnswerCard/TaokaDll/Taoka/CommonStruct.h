...
id点或页码标识块不匹配			1						
定位点检测异常				 	2-9  
（错误代码除以2向下取整所得数字为出错定位点号  错误代码为偶数表示识别出多个黑块，为奇数表示没有识别到黑块）
三个点位点之间的夹角不是90度    10
倾斜校正异常					16-31
X方向标记点检测异常				32-35
Y方向标记点检测异常				48-51（x方向异常代码-32  y方向异常代码-48  其结果为1表示没检测到矩形黑块  为2表示检测到的数量过多  为3表示检测到的数量过少）
Y方向标记点检测异常				48-63
条形码检测异常					64-79
预定义填涂点组异常				80-111
褶皱异常						112
...
#define PAGEMAKER_ERR 1					 //页码标识块不匹配
...
typedef struct AcArgs{
	...
	double seSize;                //是否调整形态学模板大小 为0不做调整  大于0则调大，小于0则调小
} AcArgs;
