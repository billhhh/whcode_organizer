#include <vector>
#include "../Taoka/CommonStruct.h"
using namespace cv;
using namespace std;
#ifndef _SRUCTDEFINE_H
#define _SRUCTDEFINE_H
//填涂点 枚举类型
 //extern enum FillingPointsType;
 //extern enum BarType;
...
typedef struct PerMarker_info{
	ushort id;        //同步头的索引号
	Rect pos;         //同步头位置    坐标原点为左上角
}PerMarker_info;

////类型三页码标识块
//typedef struct PageMarker{
//	vector<Rect> pos;   //位置 相对左上定位点的坐标
//	vector<int> graySum;   //图像反相后的灰度和
//}PageMarker;
